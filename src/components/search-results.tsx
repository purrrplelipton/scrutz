    ...
    <div className="absolute top-full right-0 left-0 z-50 mt-1 max-h-96 overflow-y-auto rounded border border-gray-300 bg-white shadow-lg">
      {isLoading ? (
        <div className="p-4 text-center text-gray-500">
          <Icon icon="svg-spinners:ring-resize" className="mr-2 inline text-xl" />
          Searching...
        </div>
      ) : results.length === 0 ? (
        <div className="p-4 text-center text-gray-500">
            ...
            <li key={result.id}>
              <button
                ...
                className={cn(
                  ...
                  selectedIndex === index ? "border-l-teal-600 bg-teal-50" : "border-l-transparent",
                )}
                  ...
                  <div className="min-w-0 grow">
                    ...
                    <div className="mt-1.5 flex items-center gap-3 text-gray-500 text-xs">
                      <span className="flex items-center gap-1">
                        <Icon icon="material-symbols:calendar-month" aria-hidden="true" />
                        <time dateTime={new Date(result.startDate).toISOString()}>
                      ...
                      <span
                        className={cn(
                          "font-medium",
                          result.campaignStatus === "Active" ? "text-green-600" : "text-red-600",
                        )}
                      >
      ...
      <div className="flex items-center justify-between border-gray-200 border-t bg-gray-50 px-4 py-2 text-gray-600 text-xs">
        <span>
          Use{" "}
          <kbd className="rounded border border-gray-300 bg-white px-1.5 py-0.5 text-xs">↑↓</kbd> to
          navigate
        </span>
        <span>
          Press{" "}
          <kbd className="rounded border border-gray-300 bg-white px-1.5 py-0.5 text-xs">Enter</kbd>{" "}
          to select
        </span>
      </div>
    </div>
